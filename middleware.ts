...
const protectedRoutes = {
  '/dashboard': ['user', 'agent', 'admin', 'super_admin'], // General dashboard accessible by all authenticated users
  '/dashboard/agent': ['agent', 'super_admin'], // Super admin can access all
  '/dashboard/admin': ['admin', 'super_admin'],
  '/dashboard/user': ['user', 'super_admin'],
...
export async function middleware(req: NextRequest) {
  ...
  if (pathname.startsWith('/dashboard/agent') && agentStatus === 'pending' && userRole === 'agent') {
  ...
  if (pathname === '/dashboard') {
    if (userRole === 'agent') {
      ...
      const superAdminUrl = new URL('/dashboard/super-admin', req.url)
